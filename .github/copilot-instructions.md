...
- **Module structure rule:** Every backend module follows the structure in `docs/developer-guide/module-structure.md`. A sub-package's `crud`/`models`/`query`/`utils` are **package-private**; siblings may import only `schema`/`contracts`/`constants`/`events`; other modules may import only `integration_service`, `contracts`, `schema`, `constants`, `events`, `dependencies`, the routers, and `subscriber_registry`. Cross-module rules are enforced by `backend/.importlinter`; the sub-package privacy rule (which import-linter cannot express) is enforced by `backend/tests/architecture/test_module_boundaries.py`. Adding a cross-package reach means adding an entry — with a reason — to `_INTRA_MODULE_SEAMS` or `_INBOUND_EXCEPTIONS` in that test.
