...
- **Auth boundary rule:** Non-auth modules must consume identity **only** through the public surface — `auth.dependencies` and `auth.identity_service.IdentityService`. The single `auth-boundary` contract in `backend/.importlinter` forbids non-auth code from importing the private `auth._internal` package (credential/token/lockout plumbing: `internal_dependencies`, `password_hasher`, `token_manager`, `security_stores`), the `auth.services` workflow layer, or any auth CRUD module (matched structurally by `auth.*.crud` / `auth.*.*.crud`). A couple of pre-existing OAuth browser-redirect-router exceptions are tracked as explicit `ignore_imports` in that file.
