  ...
  <div class="container-fluid py-4">
    ...
    <div v-if="isLoading" class="text-center py-5">
      <div class="spinner-border text-primary" role="status">
        <span class="visually-hidden">{{ $t('routesView.search_loading') }}</span>      </div>
    </div>
      ...
      <div class="col" v-for="route in routes" :key="route.id">
        <div class="card h-100 shadow-sm route-card border-0" @click="goToDetail(route.id)">
          <div class="route-preview border-bottom position-relative">
            ...
            <div class="route-preview-overlay position-absolute top-0 start-0 end-0 p-2 d-flex justify-content-between align-items-start">
          ...
          <div class="card-body route-card-body">
            ...
            <p class="card-text route-card-description small text-truncate">{{ route.description || $t('routesView.no_desc') }}</p>
          </div>
          <div class="card-footer route-card-footer d-flex justify-content-between small">
            <span><font-awesome-icon :icon="['fas', 'ruler']" /> {{ (route.distance / 1000).toFixed(2) }} km</span>
            <span v-if="route.elevation_gain"><font-awesome-icon :icon="['fas', 'mountain']" /> {{ route.elevation_gain }} m</span>
...
const initPreviewMaps = () => {
  routes.value.forEach((route) => {
    const el = document.getElementById(`map-preview-${route.id}`)
