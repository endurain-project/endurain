...
@router.get(
    "",
    response_model=health_intraday_steps_schema.HealthIntradayStepsListResponse,
    status_code=status.HTTP_200_OK,
)
async def read_health_intraday_steps_all(
    ...
    """
    Retrieve all health intraday step records for the authenticated user.

    This endpoint fetches all health intraday step entries associated with the authenticated user's ID.
...
async def read_health_intraday_steps_by_date(
    date_str: str,
    _check_scopes: Annotated[
        Callable, Security(auth_security.check_scopes, scopes=["health:read"])
    ],
    ...
    """
    Retrieve all health intraday step records for the authenticated user and given date.
...
async def read_health_intraday_steps_all_pagination(
    page_number: int,
    num_records: int,
    _check_scopes: Annotated[
        Callable, Security(auth_security.check_scopes, scopes=["health:read"])
    ...
    """
    Retrieve paginated health intraday step records for the authenticated user.

    This endpoint returns a paginated list of health intraday step data for the user identified
...
@router.post(
    "",
    response_model=health_intraday_steps_schema.HealthIntradayStepsRead,
    status_code=status.HTTP_201_CREATED,
)
async def create_health_intraday_steps(
    health_intraday_step: health_intraday_steps_schema.HealthIntradayStepsCreate,
    _check_scopes: Annotated[
...
) -> health_intraday_steps_schema.HealthIntradayStepsRead:
    """
    Create or update health intraday step data for a user.

    This endpoint creates new health intraday step data or updates existing data if an entry
    for the specified timestamp already exists. The operation is determined automatically
    based on whether steps data exists for the given timestamp.

    Args:
        health_intraday_step (health_intraday_steps_schema.HealthIntradaySteps): The health intraday step data to create
    ...
    Returns:
        health_intraday_steps_schema.HealthIntradaySteps: The created or updated health intraday step data.

    Raises:
        HTTPException: 400 error if the date field is not provided in the request.
    """
    if not health_intraday_step.timestamp:
        raise HTTPException(status_code=400, detail="Date field is required.")

    # Convert date to string format for CRUD function
    timestamp_str = health_intraday_step.timestamp.isoformat()

    # Check if health_intraday_steps for this date already exists
    steps_for_timestamp = health_intraday_steps_crud.get_health_intraday_steps_by_timerange(
        token_user_id, timestamp_str, timestamp_str, db
    )

    if steps_for_timestamp:
        # Convert to update schema with the existing ID and user_id
        health_intraday_step_update = health_intraday_steps_schema.HealthIntradayStepsUpdate(
            id=steps_for_timestamp[0].id, user_id=token_user_id, **health_intraday_step.model_dump()
        )
        # Updates the health_intraday_steps in the database and returns it
        return health_intraday_steps_crud.edit_health_intraday_steps(
            token_user_id, health_intraday_step_update, db
        )
    else:
        # Creates the health_intraday_steps in the database and returns it
        return health_intraday_steps_crud.create_health_intraday_steps(token_user_id, health_intraday_step, db)
...
async def edit_health_intraday_steps(
    health_intraday_step: health_intraday_steps_schema.HealthIntradayStepsUpdate,
    _check_scopes: Annotated[
        Callable, Security(auth_security.check_scopes, scopes=["health:write"])
    ],
    token_user_id: Annotated[
    ...
    """
    Edit health intraday step data for a user.

    This endpoint updates existing health intraday step records in the database for the authenticated user.
    Requires 'health:write' scope for authorization.

    Args:
        health_intraday_step (health_intraday_steps_schema.HealthIntradaySteps): The health intraday step data to be updated,
            containing the new values for the health intraday step record.
    ...
    Returns:
        health_intraday_steps_schema.HealthIntradaySteps: The updated health intraday step record with the new values
    ...
    """
    # Updates the health_intraday_steps in the database and returns it
    return health_intraday_steps_crud.edit_health_intraday_steps(token_user_id, health_intraday_step, db)


@router.delete(
    "/{health_intraday_step_id}", response_model=None, status_code=status.HTTP_204_NO_CONTENT
)
async def delete_health_intraday_steps(
    health_intraday_step_id: int,
    _check_scopes: Annotated[
        Callable, Security(auth_security.check_scopes, scopes=["health:write"])
    ],
    token_user_id: Annotated[
    ...
    """
    Delete a health intraday step record for the authenticated user.

    This endpoint removes a specific health intraday step entry from the database for the user
    identified by the access token. The user must have 'health:write' scope permission.

    Args:
        health_intraday_step_id (int): The unique identifier of the health intraday step record to delete.
    ...
    Raises:
        HTTPException: May be raised by dependencies if:
            ...
            - The health intraday step record doesn't exist or doesn't belong to the user
    """
    # Deletes entry from database
    health_intraday_steps_crud.delete_health_intraday_steps(token_user_id, health_intraday_step_id, db)
