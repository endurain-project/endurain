...
non-auth module from importing ``auth._internal`` directly: outside code must
consume identity through the public surface (``auth.dependencies`` and
``auth.identity_service.IdentityService``) instead.
