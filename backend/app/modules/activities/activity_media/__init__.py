...
Blobs are stored through the platform ``StorageProvider`` and served only by
the token-gated route in ``public_router``; the database holds the storage
key, never a servable path.
