...
import modules.users.users_integrations.schema as users_integrations_schema
    ...
    async def collect_and_import_activities_data_batched(
        ...
        for batch_start in range(0, len(activities_data), batch_size):
            ...
            for activity_data in activities_batch:
                ...
                # the user with one "new activity" notification per row and (b) on
                # the in-process bus render every map thumbnail, score HR zones, and
                # reverse-geocode inline during the import loop. Derived artifacts are
                # instead regenerated by the scheduled thumbnail, HR-zone, and location
                # (geocoding) backfills (the reconciliation nets), keeping the restore
                # fast and notification-free.
