    ...
    async def collect_and_import_activities_data_batched(
        ...
        for batch_start in range(0, len(activities_data), batch_size):
            ...
            for activity_data in activities_batch:
                ...
                # the in-process bus render every map thumbnail and score HR zones
                # inline during the import loop. Derived artifacts are instead
                # regenerated by the scheduled thumbnail and HR-zone backfills (the
                # reconciliation nets), keeping the restore fast and notification-free.
