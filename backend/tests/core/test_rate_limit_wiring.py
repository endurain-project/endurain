...
* the *manifest* — each sensitive endpoint carries the tier it is supposed to,
  read from the limiter's own registry rather than from the source text; and
...
# The endpoints whose tier is a security control rather than a nicety: credential
# and account-recovery flows, file ingestion, and the routes that fan out into
# third-party calls. Adding a route here is how a new sensitive endpoint gets
# covered.
_TIERED_ENDPOINTS = [
