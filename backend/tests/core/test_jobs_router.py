...
class TestReadJobsSummary:
    def test_delegates_to_crud_with_hours(self):
        fake_db = MagicMock()
        sentinel = MagicMock()
        with patch("core.jobs.router.jobs_crud.get_jobs_summary", return_value=sentinel) as get_summary:
    ...
    def test_defaults_to_24_hours(self):
        fake_db = MagicMock()
        with patch("core.jobs.router.jobs_crud.get_jobs_summary") as get_summary:
...
class TestReplayDeadLetterJob:
    def test_returns_result_when_replayed(self):
        fake_db = MagicMock()
        with patch("core.jobs.router.jobs_crud.replay_dead_letter_job", return_value=True) as replay:
    ...
    def test_404_when_not_found(self):
        fake_db = MagicMock()
        with (
            patch("core.jobs.router.jobs_crud.replay_dead_letter_job", return_value=False),
