...
class TestStoreParsedActivity:
    ...
    def test_stores_activity_and_streams(self, mock_crud, mock_streams_crud, mock_pub):
        ...
        mock_crud.create_activity = AsyncMock(return_value=created)
        ...
        result = asyncio.run(ingestion_service.store_parsed_activity(parsed, MagicMock(), MagicMock()))

        assert result is created
        mock_crud.create_activity.assert_awaited_once()
    ...
    def test_no_streams_skips_stream_creation(self, mock_crud, mock_streams_crud, mock_pub):
        ...
        mock_crud.create_activity = AsyncMock(return_value=MagicMock(id=1, user_id=1))
        mock_streams_crud.create_activity_streams = AsyncMock()

        asyncio.run(ingestion_service.store_parsed_activity(_parsed(), MagicMock(), MagicMock()))
    ...
    def test_persists_laps_sets_and_steps(self, mock_crud, mock_laps, mock_sets, mock_steps, mock_pub):
        ...
        mock_crud.create_activity = AsyncMock(return_value=MagicMock(id=9, user_id=1))

        parsed = _parsed(laps=[{"a": 1}], sets=[{"b": 2}], workout_steps=[{"c": 3}])
        asyncio.run(ingestion_service.store_parsed_activity(parsed, MagicMock(), MagicMock()))
