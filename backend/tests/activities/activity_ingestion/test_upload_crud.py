...
_MODEL = activity_ingestion_models.ActivityUploadJob
...
class TestCreateUploadJob:
    ...
    def test_commits_by_default(self, model, mock_db):
        ...
        assert result.status == activity_ingestion_schema.UploadJobStatus.PENDING
...
class TestGetUploadJob:
    def test_returns_the_job(self, mock_db):
        ...
        result = upload_crud.get_upload_job("job-1", 7, mock_db)
        assert result is not None
        assert result.id == "job-1"

    def test_returns_none_when_absent(self, mock_db):
        _stub_first(mock_db, None)
        assert upload_crud.get_upload_job("job-1", 7, mock_db) is None
    ...
    def test_maps_a_completed_job(self, mock_db):
        ...
        result = upload_crud.get_upload_job("job-1", 7, mock_db)
        assert result is not None
        assert result.status == activity_ingestion_schema.UploadJobStatus.COMPLETED
    ...
    def test_maps_a_failed_job(self, mock_db):
        ...
        result = upload_crud.get_upload_job("job-1", 7, mock_db)
        assert result is not None
        assert result.error_code == activity_ingestion_schema.UploadJobErrorCode.INVALID_FILE
...
class TestTerminalTransitions:
    ...
    def test_mark_processing(self, mock_db):
        row = _row()
        mock_db.get.return_value = row
        upload_crud.mark_processing("job-1", mock_db)
        assert row.status == "processing"

    def test_transitions_on_a_missing_row_are_no_ops(self, mock_db):
        mock_db.get.return_value = None
        upload_crud.mark_processing("gone", mock_db)
        upload_crud.mark_completed("gone", [1], mock_db)
        upload_crud.mark_failed("gone", activity_ingestion_schema.UploadJobErrorCode.INVALID_FILE, mock_db)
        mock_db.commit.assert_not_called()
