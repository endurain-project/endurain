...
def _stream(stream_id: int = 7, owner_id: int = 2) -> contracts.HrStreamForScoring:
    return contracts.HrStreamForScoring(
        stream_id=stream_id,
        activity_id=1,
        owner_id=owner_id,
        waypoints=[{"hr": 100}],
...
class TestRecomputeHrZonesForUser:
    ...
    def test_stores_the_computed_zones(self, mock_crud, mock_get_user, mock_max, mock_compute):
        from modules.activities.activity_streams import service

        hr_block = {"zone_1": {"percent": 100.0}}
        mock_compute.return_value = hr_block
        mock_crud.list_user_hr_streams.side_effect = [[_stream()], []]
        db = MagicMock()
    ...
    @patch(f"{_SVC}.users_integration_service.get_user")
    @patch(f"{_SVC}.activity_streams_crud")
    def test_clears_zones_when_max_hr_unresolvable(self, mock_crud, mock_get_user, mock_max):
    ...
    @patch(f"{_SVC}.activity_streams_crud")
    def test_a_missing_user_still_clears(self, mock_crud, mock_get_user):
        from modules.activities.activity_streams import service
...
class TestScoreActivityHrZones:
    @patch(f"{_SVC}.users_integration_service.get_user", return_value=None)
    @patch(f"{_SVC}.activity_streams_crud")
    def test_noop_when_user_missing(self, mock_crud, mock_get_user):
        from modules.activities.activity_streams import service
    ...
    @patch(f"{_SVC}.activity_streams_crud")
    def test_noop_when_no_max_hr(self, mock_crud, mock_get_user, mock_max):
        from modules.activities.activity_streams import service
    ...
    @patch(f"{_SVC}.users_integration_service.get_user", return_value=MagicMock())
    @patch(f"{_SVC}.activity_streams_crud")
    def test_noop_when_the_activity_has_no_hr_stream(self, mock_crud, mock_get_user, mock_max):
        from modules.activities.activity_streams import service
    ...
    @patch(f"{_SVC}.users_integration_service.get_user", return_value=MagicMock())
    @patch(f"{_SVC}.activity_streams_crud")
    def test_stores_the_breakdown(self, mock_crud, mock_get_user, mock_max, mock_compute):
        from modules.activities.activity_streams import service

        mock_crud.get_activity_hr_stream.return_value = _stream()
        db = MagicMock()
...
class TestBackfillMissingHrZones:
    ...
    @patch(f"{_SVC}.users_integration_service.get_user", return_value=MagicMock())
    @patch(f"{_SVC}.activity_streams_crud")
    def test_scores_missing_streams(self, mock_crud, mock_get_user, mock_max, mock_compute):
        from modules.activities.activity_streams import service

        mock_crud.list_hr_streams_missing_zones.side_effect = [[_stream(stream_id=5)], []]
    ...
    @patch(f"{_SVC}.users_integration_service.get_user", return_value=MagicMock())
    @patch(f"{_SVC}.activity_streams_crud")
    def test_skips_when_the_owner_has_no_max_hr(self, mock_crud, mock_get_user, mock_max):
    ...
    @patch(f"{_SVC}.users_integration_service.get_user", return_value=MagicMock())
    @patch(f"{_SVC}.activity_streams_crud")
    def test_each_owner_is_looked_up_once_per_run(self, mock_crud, mock_get_user, mock_max, mock_compute):
        from modules.activities.activity_streams import service

        mock_crud.list_hr_streams_missing_zones.side_effect = [
            [_stream(stream_id=5, owner_id=2), _stream(stream_id=6, owner_id=2)],
            [],
        ]
